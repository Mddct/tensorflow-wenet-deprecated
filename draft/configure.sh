...
" Y or enter for CPU (tensorflow-macos), N for GPU (tensorflow). [Y/n] " INPUT
  case $INPUT in
...
if [[ "$TF_NEED_CUDA" == "0" ]]; then
  ...
  if [[ $(${PIP} show tensorflow-macos) == *tensorflow-macos* ]] || [[ $(${PIP} show tf-nightly-cpu) == *tf-nightly-cpu* ]] ; then
  ...
  # Check if it's installed
   if [[ $(${PIP} show tensorflow) == *tensorflow* ]] || [[ $(${PIP} show tf-nightly) == *tf-nightly* ]]; then
    ...
    if [[ $(${PIP} show tensorflow-macos) == *tensorflow-macos* ]]; then
