...
""" This code modified from lingvo
    https://github.com/tensorflow/lingvo/blob/master/lingvo/core/spectrum_augmenter.py
"""
...
#     https://arxiv.org/pdf/1904.08779.pdf bd
#     """
class SpectrumAugmenter():
    default_params = {
        "dtype": tf.float32,
        ...
        # Whether to noisify the time masked region
        "use_noise": False,
        ...
        # Number of warm-up steps, in which the
    ...
    def __init__(self) -> None:
        self.params = SpectrumAugmenter.default_params
    ...
    def _GetMask(self,
        ...
        masked_portion = random_uniform(shape=(batch_size, multiplicity),
                                        ...
                                        dtype=dtype,
                                        seed=seed_1)
        masked_frame_size = EinsumBBmBm(max_length, masked_portion)
        ...
        if masks_per_frame > 0:
            ...
            pre_mask = EinsumBmtBmBt(pre_mask, multiplicity_weights)
        else:
    ...
    def _ConstructWarpMatrix(self, batch_size, matrix_size, origin,
        ...
        x = (EinsumBBmBm(slope_0, x) +
             EinsumBBmBm(slope_1 - slope_0, tf.nn.relu(x - destination_bc)) +
             EinsumBBmBm(slope_2 - slope_1, tf.nn.relu(x - choose_range_bc)))
    ...
    def _FrequencyMask(self, inputs, global_seed, dtype=tf.float32):
        ...
        block_arrays = self._GetMask(tf.shape(inputs)[0],
                                     ...
                                     dtype=dtype,
                                     max_ratio=1.0)
        return EinsumBxycByBxyc(inputs, block_arrays)
    ...
    def _TimeMask(self,
        ...
        outputs = EinsumBxycBxBxyc(inputs,
                                   block_arrays,
        ...
        if noisify:
            ...
            outputs_mask = EinsumBxyBxBxy(noise,
                                          1.0 - block_arrays,
    ...
    def _TimeWarp(self, inputs, seq_lengths, global_seed, dtype=tf.float32):
        ...
        return EinsumBxycBzxBzyc(inputs, warp_matrix, name='einsum_forwarping')
