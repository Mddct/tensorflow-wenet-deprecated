"""code copy from https://github.com/tensorflow/models/blob/master/official/nlp/modeling/ops/beam_search.py"""
...
class SequenceBeamSearch(tf.Module):
    ...
    def search(self, initial_ids, initial_cache):
        ...
        def _grow_alive_seq(state):
            ...
            if self.padded_decode:
                ...
                flat_ids = flatten_beam_dim(
                    alive_seq)  # [batch_size * beam_size]
            ...
            logits = _unflatten_beam_dim(flat_logits, batch_size,
                                         self.beam_size)
            ...
            if self.padded_decode:
                ...
                topk_seq = tf.tensor_scatter_nd_update(
                    topk_seq, [[i + 1]], tf.expand_dims(topk_ids, axis=0))
        ...
        def _search_step(state):
            ...
            new_seq, new_log_probs, topk_ids, new_cache = _grow_alive_seq(
                state)
            ...
            finished_state = _get_new_finished_state(state, new_seq,
                                                     new_log_probs,
                                                     new_finished_flags)
            ...
            for inner_value in tf.nest.flatten(value):
                if inner_value.dtype != self.dtype:
                    raise TypeError(
                        ...
                        (key, inner_value.dtype.name, self.dtype.name,
                         inner_value))
    ...
    def _continue_search(self, state):
        ...
        best_alive_scores = tf.squeeze(tf.slice(alive_log_probs, [0, 0],
                                                [-1, 1]),
